...
// 31 fps
static void render_8bit_tiles_using_one_tile_height_buffer(const unsigned tile_dx) {
  ...
  for (unsigned y = 0; y < frame_height; y += tile_height) {
    ...
    for (unsigned ty = 0; ty < tile_height; ty++) {
      if (tile_width_minus_dx) {
        ...
        const uint8_t* tile_data_ptr = tiles8bit[0].data + (ty * tile_height) + tile_dx;
      ...
      for (unsigned tx = 1; tx < frame_width / tile_width; tx++) {
        const uint8_t* tile_data_ptr = tiles8bit[0].data + (ty * tile_height);
...
void loop() {
  ...
  render_8bit_tiles_using_one_tile_height_buffer(tile_dx);
  tft.endWrite();
